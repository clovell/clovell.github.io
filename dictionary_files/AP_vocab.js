(function() {
    ...
    //need to rewrite to automatically use headers as data fields
    ...
    async function initialize() {
        ...
        try {
            const response = await fetch('https://clovell.github.io/dictionary_files/AP_vocab.csv');
            if (!response.ok) { throw new Error(`HTTP error! status: ${response.status}`); }
            const csvText = await response.text();
            vocabulary = parseCSV(csvText);
        } catch (error) {
            console.error("Error fetching or parsing AP_vocab.csv:", error);
            resultDisplay.innerHTML = `<div class="placeholder-text"><p style="color:var(--danger-color);">Error: Could not load dictionary.csv. This app must be run from a web server.</p></div>`;
            return;
        }
